...
In this example, I use linear regression and k-nearest neighbors for imputation. This example uses a dataset containing several variables measuring rivers' CO2 emissions. I don't have the original documentation outlining what each variable actually is, but I'm guessing most are just what they sound like - temp is water temp in Celcius, depth is depth of the river, etc. Flow? Maybe how much water flows through the river? Must be different than velocity, so this `flow` is the one where I'm least certain. Regardless, I think we know enough to do some analysis.
...
The first step here is to fit a linear model. In previous classwork we explored the data, poked around for outliers, etc. so I feel okay going right into the linear model here. I ran through a few different options but this ended up being my best model:
...
```
# Imputation Based Off Linear Model
...
```{r}
# This bit requires the user to install the package YaleToolkit.
...
whatis(y)
```
And now we can see that those 67 values of `temp` are no longer missing. Huzzah!

Next, I'll use a k-nearest-neighbor approach to imputation for the variable `temp`.

# K Nearest Neighbors: A Single River
...
## four variables to solve this part of the challenge. The row indices are ## gathered corresponding to the missing values for temp:
...
So without looking too hard I see that the first row with missing values for `temp` is row 5 of `y`, corresponding to observed information in row 5 of `z`.

Now I was asked to calculate the "distance" between row five and every 
other row, saving these distances in the new variable `delta`.  
PROF HINT: delta[5] should be 0 after you do the calculation!
...
Next, I was asked to identify 5 rivers (or more than 5 if tied distances in delta) "closest to" this one river (row 5, which I think is just a coincidence). Then I use the mean value of `temp` from this group of "close" rivers to fill in the missing value of y$temp[5].
...
y$temp[5] # so looks like that worked, which is always neat
...
```
# K Nearest Neighbors: All the Rivers

Now to solve the full problem, I'll do this in a loop until I have imputed all 67 missing values of `temp`. (Note: `temp.orig` will still have 67 missing values.)
...
Now we have no missing values for `temp`; they've all been imputed with values based off of their 5 nearest neighbors. 
